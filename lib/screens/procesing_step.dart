import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_svg/svg.dart';
import 'package:lottie/lottie.dart';
import 'package:mihome_app/blocs/blocs.dart';
import 'package:reactive_forms/reactive_forms.dart';

import '../app_styles.dart';
import '../models/models.dart';

class ProcessingStep extends StatefulWidget {
  final Agenda ot;
  final FormGroup? formulario;
  final String step;
  final List<EquipoInstalado> equipos;

  const ProcessingStep({
    Key? key,
    required this.ot,
    required this.step,
    this.formulario,
    this.equipos = const [],
  }) : super(key: key);

  @override
  State<ProcessingStep> createState() => _ProcessingStepState();
}

class _ProcessingStepState extends State<ProcessingStep> {
  late OTStepBloc _stepBloc;

  @override
  void initState() {
    _stepBloc = BlocProvider.of<OTStepBloc>(context);

    switch (widget.step) {
      case 'LOCALIZAR':
        _stepBloc.localizarOT(ot: widget.ot);
        break;
      case 'ACTIVAR':
        _stepBloc.activarOT(
          ot: widget.ot,
          formulario: widget.formulario!,
          equipos: widget.equipos,
        );
        break;
      case 'CERTIFICAR':
        _stepBloc.certificarOT(
          ot: widget.ot,
          formulario: widget.formulario!,
        );
        break;
      case 'LIQUIDAR':
        _stepBloc.liquidarOT(
          ot: widget.ot,
          formulario: widget.formulario!,
        );
        break;
      default:
    }

    super.initState();
  }

  @override
  @override
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async => false,
      child: Scaffold(
        body: Container(
          height: MediaQuery.of(context).size.height * 0.7,
          width: MediaQuery.of(context).size.width * 0.9,
          alignment: Alignment.center,
          padding: const EdgeInsets.all(10),
          decoration: const BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.all(
              Radius.circular(32),
            ),
          ),
          child: BlocBuilder<OTStepBloc, OTStepState>(
            builder: (context, state) {
              return Column(
                mainAxisAlignment: MainAxisAlignment.center,
                crossAxisAlignment: CrossAxisAlignment.center,
                children: [
                  Lottie.asset(
                    'assets/lottie/processing.json',
                    width: 200,
                    height: 250,
                    animate: true,
                  ),
                  Container(
                    width: MediaQuery.of(context).size.width * 0.7,
                    height: 200,
                    margin: const EdgeInsets.only(top: 15),
                    padding: const EdgeInsets.all(12),
                    decoration: const BoxDecoration(
                      color: Colors.white,
                      borderRadius: BorderRadius.all(
                        Radius.circular(10),
                      ),
                      boxShadow: [
                        BoxShadow(
                          color: Color(0xffcccaaa),
                          blurRadius: 5,
                          offset: Offset(0, 2), // changes position of shadow
                        ),
                      ],
                    ),
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Row(
                          children: [
                            SvgPicture.asset(
                              !state.guardado
                                  ? 'assets/Iconos/warning.svg'
                                  : 'assets/Iconos/done.svg',
                              height: 20,
                            ),
                            const Text(
                              " Guardado localmente: ",
                              style: TextStyle(
                                fontFamily: 'CronosLPro',
                                fontSize: 16,
                                color: kSecondaryColor,
                              ),
                            ),
                            Text(
                              state.guardado ? "Exitoso" : "En proceso...",
                              style: const TextStyle(
                                fontFamily: 'CronosLPro',
                                fontSize: 16,
                                color: kSecondaryColor,
                              ),
                            ),
                          ],
                        ),
                        const SizedBox(
                          height: 5,
                        ),
                        Row(
                          children: [
                            SvgPicture.asset(
                              state.enviado
                                  ? 'assets/Iconos/done.svg'
                                  : 'assets/Iconos/warning.svg',
                              height: 20,
                            ),
                            const Text(
                              " Enviado: ",
                              style: TextStyle(
                                fontFamily: 'CronosLPro',
                                fontSize: 16,
                                color: kSecondaryColor,
                              ),
                            ),
                            Text(
                              (state.enviado && state.guardado)
                                  ? "Exitoso"
                                  : "En Proceso...",
                              style: const TextStyle(
                                fontFamily: 'CronosLPro',
                                fontSize: 16,
                                color: kSecondaryColor,
                              ),
                            ),
                          ],
                        ),
                        const SizedBox(
                          height: 5,
                        ),
                        Row(
                          children: [
                            const Text(
                              "Observación: ",
                              style: TextStyle(
                                fontFamily: 'CronosSPro',
                                fontSize: 16,
                                color: kSecondaryColor,
                              ),
                            ),
                            Flexible(
                              child: Text(
                                state.mensaje,
                                style: const TextStyle(
                                    fontFamily: 'CronosLPro',
                                    fontSize: 16,
                                    color: kSecondaryColor,
                                    overflow: TextOverflow.ellipsis),
                                maxLines: 5,
                              ),
                            ),
                          ],
                        ),
                      ],
                    ),
                  ),
                  const SizedBox(
                    height: 10,
                  ),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [
                      state.mensaje == 'Envio fallido'
                          ? MaterialButton(
                              elevation: 4,
                              color: Colors.white,
                              onPressed: () async {
                                switch (widget.step) {
                                  case 'LOCALIZAR':
                                    await _stepBloc.retryLocalizarOT(
                                        ot: widget.ot.ot.toString());
                                    break;
                                  case 'ACTIVAR':
                                    await _stepBloc.retryActivarOT(
                                      ot: widget.ot.ot.toString(),
                                      equipos: widget.equipos,
                                    );
                                    break;
                                  case 'CERTIFICAR':
                                    await _stepBloc.retryCertificarOT(
                                      ot: widget.ot.ot.toString(),
                                    );
                                    break;
                                  case 'LIQUIDAR':
                                    await _stepBloc.retryLiquidarOT(
                                      ot: widget.ot.ot.toString(),
                                    );
                                    break;
                                  default:
                                }
                              },
                              child: const Text(
                                "Intentar de Nuevo",
                                style: TextStyle(
                                  color: kSecondaryColor,
                                  fontFamily: 'CronosLPro',
                                  fontSize: 16,
                                ),
                              ),
                            )
                          : Container(),
                      MaterialButton(
                        elevation: 4,
                        color: Colors.white,
                        onPressed: () {
                          Navigator.pop(context);
                        },
                        child: const Text(
                          "Cerrar",
                          style: TextStyle(
                            color: kSecondaryColor,
                            fontFamily: 'CronosLPro',
                            fontSize: 16,
                          ),
                        ),
                      ),
                    ],
                  ),
                ],
              );
            },
          ),
        ),
      ),
    );
  }
}
